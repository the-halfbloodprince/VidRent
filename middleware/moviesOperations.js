...
async function postMovies(req,res) {
    ...
    Genre.findById(req.body.genreId).then((genre)=>{
        ...
        movie.save().then((movie)=>res.send(movie))
                    .catch((err)=>res.status(500).send('Failed to save'))
    })
...
async function putMovies(req,res) {
    ...
    Movie.findByIdAndUpdate(req.params.id,{
        ...
        .catch((err)=>res.status(404).send('Movie with the given ID was not found'))
}
...
async function deleteMovies(req,res) {
    ...
    Movie.findByIdAndDelete(req.params.id).then((movie)=>res.send(movie))
                                            .catch((err)=>res.status(404).send('Movie with the given ID was not found'))
}

//Check this and implement it elsewhere too
async function getMovie(req,res) {
    Movie.findById(req.params.id).then((movie)=>res.send(movie))
                                .catch((err)=>{
